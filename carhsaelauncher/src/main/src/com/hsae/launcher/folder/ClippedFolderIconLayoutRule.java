...
public class ClippedFolderIconLayoutRule {
    private static final String TAG = "ClippedFolderIconLayoutRule";
...
//        }
        ...
        int colIndex = getCol(tempIndex);
        int rowIndex = getRow(tempIndex);
...
//        transX = padl + mIconSize * MIN_SCALE * getCol(tempIndex) + getCol(tempIndex) * 5 -10;
      // transY =  padt + mIconSize * MIN_SCALE * (getRow(tempIndex) - 1) + (getRow(tempIndex) - 1) * 5 -10;
        /* 20190924 modify end */

        if (params == null) {
            params = new PreviewItemDrawingParams(transX, transY, totalScale, overlayAlpha);
        } else {
           // Log.i(TAG, "**************hxq======== transX[" + transX + "], transY["+ transY +"],totalScale["+ totalScale+"]");
